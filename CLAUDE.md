...
app/technology/…    → components/technology/…          5-layer architecture
```
...
**`/technology`** is four sections, in the order an engineer needs them:

```
Hero → ArchitectureV2 → CognitionV2 → ProductTech → BoundaryV2
```

- **`ArchitectureV2`** — the five-layer stack drawn as an architecture diagram: named components inside each band, the product pills that implement it, and the read/write loop. Generation is folded into layer 04 (`Agent OS`) rather than standing alone. Both loop rails terminate at layer 03, so the picture says *the loop closes at the ontology* without a caption. Layers 03 and 04 carry the lime cap because they are the two allmeta supplies — the colour does the attribution, so don't add a legend. The frontier model docks in as `--c-info` and the human decision as `--c-violet`, the same two tokens with the same two meanings `DualityV2` and `HarnessV2` use on the home page.
- **`CognitionV2`** — *Agent = ontology + model + harness*. Every harness stage carries the ontology element it derives from, because the claim is that the harness is **compiled, not hand-written per agent** — but at **two confidences**, and the drawing must keep them apart: action contracts, preconditions and permission scope compile totally (solid arrow, lime source tag); context policy, the post-state check and the trace are *typed* by the ontology and ship with the role (dashed arrow, neutral tag). Stating all six at one confidence is the over-claim an engineer catches first. The model is drawn *inside* the harness and the harness wall *is* the tool boundary: a denied action dies at an amber strike on it. Two identical `本体 v·` stamps (ontology, harness) are the version-pinning argument drawn. Careful: "harness" means two different things on this site. `HarnessV2` (home) is the org-level decision surface, 决策中枢 — its English now says "decision surface"; this is per-agent runtime scaffolding, 骨架. Reserve the English word "harness" for the per-agent layer.
- **Vocabulary that must not come back** (an AI-architecture review removed each for a reason): *回放 / replay* of a model run — non-determinism makes re-execution impossible; say 审计重建 / audit reconstruction, and use 重放 only for regression against recorded tool responses. *结果复核* — reads as a human second review or a model grading a model; say 写回核验 / post-state check. *不用重新发版* — describes an ungated blast radius; the claim is pinned-per-run propagation with a gated promotion. *事件总线治理* — a bus observes, it governs nothing; governance is the permission gate, the rule preconditions and the human node. *Remove every human-judgment break-point* — contradicts 判断不可外包; say auto-pilot *up to the decision*.
- **`ProductTech`** — the six products as engineering: fixed technology / architecture / advantage rows so they're comparable by scanning a column.
- **`BoundaryV2`** — "behind your wall, the whole system; outside it, one model endpoint". This is the section that pays the hero's 自主可控 debt, and it does so with **a deployment drawing, not adjectives** — the words 自主可控 / 信创 / 主权 do not appear in it. One perimeter (the customer's host), the app and its three stores inside, exactly one aperture (the model gateway), a reach the model does not have dying at an amber strike. A self-hosted model is drawn *inside* the wall in `--c-info`: still borrowed capability, because sovereignty is about where judgement, action and record sit, not where the weights sit — and the same socket serves both, which is the swap claim drawn. Every string is scoped to something checkable in the repos, and **"one opening" is a run-time claim only** (the ontology generator calls a web-search API at design time). Careful: "wall / 墙" here is the deployment boundary; `DualityV2`'s perimeter is conceptual. They rhyme on purpose and are not the same line.

Both diagrams render a purpose-built narrow variant below `lg` rather than shrinking the wide one.
